module Task3_1 where
    ...
    add  Zero wpn2 = wpn2
    add  (Succ x) wpn2 = Succ (add x wpn2)
    add  (Pred x) wpn2 = Pred (add x wpn2)
    ...
    wpnQuotRem wpn1 wpn2 = case (wpn1, wpn2) of
        ...
        otherwise -> (res, wpn1 - wpn2 * res)
            where s | wpn2 > 0 = wpn1
                    | wpn2 < 0 = negate wpn1
                  res = wpnQuotRem' (abs wpn1) (abs wpn2) s
    
    wpnQuotRem' :: WeirdPeanoNumber -> WeirdPeanoNumber -> WeirdPeanoNumber -> WeirdPeanoNumber
    wpnQuotRem' wpn1 wpn2 s | wpn1 >= wpn2 = if (s < 0 )
        then Pred (wpnQuotRem' (wpn1 - wpn2) wpn2 s)
        else Succ (wpnQuotRem' (wpn1 - wpn2) wpn2 s)
                            | otherwise = Zero
